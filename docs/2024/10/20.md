...
description: 本期日报包含 C++ 模板、概念、数据结构和性能优化等多个话题
__tags__:
- title: '## 1. 编译期判断类型是否是类模板的实例'
  score: 7/10
  tags:
  - C++
  - 编译期
  ...
  - 专家知识
  - 需改进
  explanation: 介绍编译期判断类型是否为类模板实例的技术，涉及模板特化和类型推导，属于专家知识。需改进：示例代码对非类型模板参数无效，说明不够全面。
- title: '## 2. concept 实例化失败'
  score: 8/10
  tags:
  - C++
  - 语法特性
  - 泛型编程
  - 专家知识
  explanation: 讨论 concept 实例化失败时的行为，解释标准中的非良构不要求诊断规则，属于专家知识。
- title: '## 3. 哈希表怎么处理冲突'
  score: 9/10
  tags:
  - C++
  ...
  - 算法
  - 进阶知识
  explanation: 比较哈希表冲突处理的不同方法，分析 std::unordered_map 的拉链法和开放寻址法的性能差异，涉及性能优化和算法，属于进阶知识。
- title: '## 4. 为什么 std::map 用 rbtree 不用 btree'
  score: 8/10
  tags:
  - C++
  - STL
  ...
  - 进阶知识
  - 内容较主观
  explanation: 分析 std::map 使用红黑树而非 B 树的历史原因和性能考量，讨论缓存友好性，涉及性能优化和硬件相关，属于进阶知识。内容较主观：包含历史局限性的主观判断。
- title: '## 5. 为什么以前的人不注重缓存'
  ...
  - 进阶知识
  - 内容较主观
  explanation: 探讨历史上对缓存重视程度不足的原因，分析性能感知的时间延迟效应，涉及性能优化和硬件相关，属于进阶知识。内容较主观：包含历史原因的主观推测。
- title: '## 6. 面试题：cache 和 buffer 的区别'
  score: 6/10
  tags:
  - 基础知识
  - 面试常见
  - 其他
  - 需改进
  explanation: 解释 cache 和 buffer 的基本概念区别，属于基础知识和面试常见话题。需改进：内容较为简略，缺乏深入的技术分析。
---
