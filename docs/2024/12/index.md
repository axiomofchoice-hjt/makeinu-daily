...
next:
  ...
  link: /2025/01
description: 本期日报包含 C++ 语言特性、标准库、编译链接、工程实践等多个话题
__tags__:
- title: '## 1. `auto&& t = T{};` 是不是安全的'
  score: 8/10
  tags:
  - C++
  - 语法特性
  - 内存管理
  - 基础知识
  explanation: 讨论临时对象的生命周期延长机制，涉及右值引用和 const 引用的行为对比，属于 C++ 基础语法和内存管理知识。
- title: '## 2. `"-Wl,--icf=all"` 运行时不符合预期'
  score: 7/10
  tags:
  - C++
  - 编译构建和调试
  - 性能优化
  - 进阶知识
  explanation: 分析链接器选项导致函数合并的问题，涉及模板膨胀和二进制优化，属于编译构建和性能优化领域的进阶知识。
- title: '## 3. `std::function` 递归函数开销太大导致 leetcode 题目超时'
  score: 8/10
  tags:
  - C++
  - 标准库特性
  ...
  - 性能优化
  - 进阶知识
  explanation: 探讨 std::function 的类型擦除和内存分配开销，对比 lambda 递归的优化方案，涉及函数对象和性能优化，属于进阶知识。
- title: '## 4. `std::hive` 容器'
  score: 7/10
  tags:
  - C++
  ...
  - 内存管理
  - 进阶知识
  explanation: 介绍未来可能进入 C++26 的 std::hive 容器，分析其对象池实现原理和性能特性，属于 STL 和内存管理的进阶知识。
- title: '## 5. 一个经典错误：`std::vector` 扩容导致引用失效'
  score: 9/10
  tags:
  - C++
  - STL
  ...
  - 基础知识
  - 需改进
  explanation: 分析 vector 扩容导致引用失效的经典问题，涉及求值顺序未指定和迭代器失效，代码示例存在潜在问题需要改进。
- title: '## 6. 神经网络相同输入会得到不同计算结果'
  score: 6/10
  tags:
  - 其他
  - 并发编程
  - 内容较主观
  explanation: 讨论多线程原子操作导致的浮点计算误差，内容较主观且不完全针对 C++，属于其他话题。
- title: '## 7. `std::enable_shared_from_this` 必须公开继承，否则调用会抛异常'
  score: 8/10
  tags:
  - C++
  - 标准库特性
  ...
  - 面向对象编程
  - 进阶知识
  explanation: 揭示 enable_shared_from_this 继承权限的陷阱，分析其实现原理和检测方法，涉及智能指针和面向对象编程，属于进阶知识。
- title: '## 8. C++ module 可以用了吗'
  score: 7/10
  tags:
  - C++
  - 编译构建和调试
  - 工程实践
  - 进阶知识
  explanation: 评估 C++ module 的可用性和编译速度优势，对比预编译头文件，属于编译构建和工程实践的进阶知识。
- title: '## 9. `std::conditional_t` 会对未命中的 Type 做推导吗'
  score: 8/10
  tags:
  - C++
  - 语法特性
  ...
  - 编译期
  - 专家知识
  explanation: 深入分析 conditional_t 的实例化行为，提供 constexpr if 和延迟实例化的解决方案，涉及模板元编程，属于专家知识。
- title: '## 10. 万能引用不匹配初始化列表'
  score: 7/10
  tags:
  - C++
  - 语法特性
  - 泛型编程
  - 进阶知识
  explanation: 探讨万能引用对初始化列表的匹配问题，涉及数组引用和类型推导，属于泛型编程的进阶知识。
- title: '## 11. 宏里的变量名和宏外一致导致 bug'
  score: 8/10
  tags:
  - C++
  - 预处理和宏
  - 工程实践
  - 基础知识
  explanation: 分析宏展开导致的变量名冲突问题，强调宏的命名空间缺陷和解决方案，属于预处理和工程实践的基础知识。
- title: '## 12. 宏有哪些不可替代的功能'
  score: 7/10
  tags:
  - C++
  - 预处理和宏
