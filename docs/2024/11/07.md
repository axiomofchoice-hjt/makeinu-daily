...
description: 本期日报包含 C++ 标准库特性和设计模式的话题
__tags__:
- title: '## 1. `std::vector<bool>` 的坑'
  score: 9/10
  tags:
  - C++
  - 标准库特性
  ...
  - 基础知识
  - 需改进
  explanation: 介绍 std::vector<bool> 的特殊实现导致的代理对象问题，涉及标准库特性和内存管理，属于基础知识。需改进：没有提到这是
    C++ 标准明确规定的特化，而非实现缺陷。
- title: '## 2. 设计模式其实就是一些套路'
  score: 7/10
  tags:
  - 工程实践
  - 内容较主观
  - 基础知识
  - 其他
  explanation: 讨论设计模式在软件开发中的本质和应用，观点较主观，属于工程实践的基础知识。
---
