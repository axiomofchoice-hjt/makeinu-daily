...
description: 本期日报包含 10 个 C++ 及相关话题，涵盖 STL 设计、未定义行为、编程实践、性能优化、语法风格等。
__tags__:
- title: '## 1. 为什么 STL 迭代器是左闭右开区间'
  ...
  - STL
  - 基础知识
  explanation: 解释 STL 迭代器采用左闭右开区间设计的历史原因和优点，并提供了延伸阅读链接。属于 C++ 基础知识。
- title: '## 2. 未定义和未指定有什么区别'
  score: 8/10
  tags:
  - C++
  - 基础知识
  - 需改进
  explanation: 区分未定义行为 (UB) 和未指定行为，并举例说明。但部分表述（如“买麦当劳套餐”）较主观，且未深入解释 C++26 对未初始化变量读取行为的新分类
    (erroneous behavior)。
- title: '## 3. “防御性编程”鉴赏：shared_ptr 类型的成员变量，在成员函数里复制一份保证存活'
  ...
  - 内容较主观
  - 进阶知识
  explanation: 批评一种不必要的 shared_ptr 复制做法，认为其是掩盖问题的“防御性编程”。观点较主观，但涉及 C++ 内存管理和多线程安全的基本原理，属于进阶知识。
- title: '## 4. 裸指针可以用 `optional<T&>` `span` `mdspan` 替代了'
  score: 8/10
  tags:
  - C++
  - 标准库特性
  - 内存管理
  - 进阶知识
  explanation: 介绍现代 C++ (C++20/23/26) 中用于替代裸指针特定用途的新工具，如 optional<T&>, span, mdspan。属于
    C++ 标准库特性和内存管理话题，是进阶知识。
- title: '## 5. 性能和指令数有关系吗'
  score: 9/10
  tags:
  - C++
  ...
  - 硬件相关
  - 进阶知识
  explanation: 明确说明性能与指令数无直接关系，强调 CPU 流水线、缓存等因素的重要性，并指出评估性能必须依赖基准测试。涉及性能优化和硬件知识，属于进阶知识。
- title: '## 6. 为什么不建议过早优化'
  score: 8/10
  tags:
  - C++
  ...
  - 内容较主观
  - 基础知识
  explanation: 讨论过早优化的弊端，主张在保证可读性的前提下针对热点进行优化。观点带有一定主观性，属于工程实践和基础知识。
- title: '## 7. shallow const 问题（浅 const 问题）'
  score: 8/10
  tags:
  - C++
  - 语法特性
  - 内存管理
  - 进阶知识
  explanation: 解释 C++ 中 const 修饰指针时的“浅 const”现象，并提供了解决方案（如重载成员函数）和其他语言的对比。涉及 C++ 语法特性和内存管理，是进阶知识。
- title: '## 8. 写 `int* p;` 还是 `int *p;`'
  score: 7/10
  tags:
  - C++
  - 工程实践
  - 内容较主观
  - 基础知识
  explanation: 讨论指针声明的两种风格，分析其历史原因和直观性，并给出个人建议（靠左 `int* p;`）。内容较主观，属于代码风格相关的工程实践和基础知识。
- title: '## 9. “可执行”的 C++ 源文件'
  score: 6/10
  tags:
  - C++
  ...
  - 冷门知识
  - 其他
  explanation: 展示一个结合 Shell 脚本实现自编译自执行的 C++ 源文件技巧。这是一个冷门的、与编译构建相关的趣味实践，标签“其他”表示其特殊性质。
- title: '## 10. 推荐阅读'
  score: 8/10
  tags:
  - C++
  ...
  - 其他
  - 需改进
  explanation: 推荐了四篇外部文章/视频，涉及编程语言吐槽、模板理解、行业报告和错误处理。但推荐理由简略，对其中一篇（“最烂编程语言”）仅提示“结合大佬的吐槽来看”，未提供自身评价，信息不够全面。标签“其他”涵盖非直接技术讲解的推荐内容。
---

![img](/img/2025-12-index.webp)
