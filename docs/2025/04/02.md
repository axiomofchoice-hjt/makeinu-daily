...
description: 本期日报包含 C++ 继承内存布局、CRTP 约束和用途等话题
__tags__:
- title: '## 1. 继承中的 public private 影响内存布局'
  score: 9/10
  tags:
  - C++
  - 内存管理
  ...
  - 专家知识
  - 冷门知识
  explanation: 讨论 C++ 继承中访问修饰符对内存布局的影响，涉及 Itanium ABI 和标准布局规则，属于内存管理和面向对象编程的专家级冷门知识。
- title: '## 2. CRTP 基类不能用 requires 对子类模板做约束'
  score: 8/10
  tags:
  - C++
  - 泛型编程
  - 编译期
  - 专家知识
  explanation: 分析 CRTP 基类使用 requires 约束子类模板时的编译错误，涉及不完整类型和约束放置位置，属于泛型编程和编译期的专家知识。
- title: '## 3. CRTP 有什么用'
  score: 7/10
  tags:
  - C++
  ...
  - 工程实践
  - 进阶知识
  explanation: 介绍 CRTP 的设计模式用途，包括 std::enable_shared_from_this 和 visitor 模式应用，属于泛型编程和工程实践的进阶知识。
---
