...
bug 是静态局部变量只有首次调用会初始化，第二次调用就寄了。
...
不行。因为 rhs 传参的时候，它需要被初始化。而“初始化”这个过程是依赖构造函数，这样就会无限递归。
...
因为一般 CPU 的硬件预取能覆盖常见的访问模式，不用再做软件预取了。在简单的 CPU 上软件预取就可以看到区别。

第二点就是算法的访问模式了，特殊的算法加预取会有提升。

## 6. float 转换为相同二进制表示的 int

标准做法是 `std::bit_cast`（C++20），旧版本用 `memcpy`。不用担心性能，memcpy 第三个参数是常量会被编译器优化掉。
...
不标准且不推荐用 `reinterpret_cast`（有类型别名问题）。
