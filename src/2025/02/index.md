...
next:
  ...
  link: /2025/03
description: 本期日报包含 C++ 语言相关的多个话题，涵盖内存管理、性能优化、并发编程、标准库特性等
__tags__:
- title: '## 1. 运行报错 "pure virtual method called"'
  score: 9/10
  tags:
  - C++
  - 面向对象编程
  ...
  - 基础知识
  - 需改进
  explanation: 分析纯虚函数在构造析构期间调用的未定义行为，涉及虚函数表和 ABI 实现细节，属于基础知识。需改进：可补充更多实际场景示例。
- title: '## 2. `stdx::simd` 默认使用什么指令集'
  score: 8/10
  tags:
  - C++
  - 标准库特性
  ...
  - 性能优化
  - 进阶知识
  explanation: 讨论 stdx::simd 的默认指令集选择和与 native_simd 的区别，涉及硬件特性和性能优化，属于进阶知识。
- title: '## 3. 内存序 AcqRel，实际运行各个核会看到不一样的顺序吗'
  score: 7/10
  tags:
  - C++
  - 并发编程
  - 硬件相关
  - 专家知识
  explanation: 探讨内存序在多核架构下的可见性问题，涉及缓存一致性和硬件特性，属于专家知识。
- title: '## 4. 内存序先写后读的问题'
  score: 8/10
  tags:
  - C++
  - 并发编程
  - 基础知识
  explanation: 分析原子操作中内存序对指令重排的影响，解释为何需要 seqcst 内存序，属于基础知识。
- title: '## 5. 标记访问某个变量时必须持有某个锁'
  score: 7/10
  tags:
  - C++
  - 工程实践
  - 并发编程
  - 进阶知识
  explanation: 介绍 Clang Thread Safety Analysis 工具，用于标记线程安全约束，属于工程实践和进阶知识。
- title: '## 6. magic static'
  score: 7/10
  tags:
  - C++
  - 基础知识
  - 内容较主观
  explanation: 解释静态局部变量的初始化特性和常见误用，涉及单例模式实现，属于基础知识。内容较主观：包含个人观点。
- title: '## 7. 拷贝构造的参数不加引用'
  score: 9/10
  tags:
  - C++
  - 语法特性
  - 基础知识
  explanation: 说明拷贝构造函数参数必须为引用的原因，避免无限递归，属于语法特性和基础知识。
- title: '## 8. 为什么 L1 Cache 只有页内的位才能索引'
  score: 8/10
  tags:
  - 硬件相关
  - 专家知识
  explanation: 解释虚拟地址到物理地址映射对缓存索引的影响，涉及缓存一致性，属于硬件相关和专家知识。
- title: '## 9. 加 prefetch 性能是否有提升'
  score: 7/10
  tags:
  - C++
  - 性能优化
  ...
  - 工程实践
  - 进阶知识
  explanation: 讨论软件预取的实际效果和适用场景，涉及性能优化和硬件特性，属于工程实践和进阶知识。
- title: '## 10. float 转换为相同二进制表示的 int'
  score: 8/10
  tags:
  - C++
  - 标准库特性
  - 类型
  - 基础知识
  explanation: 比较不同方法实现 float 到 int 的二进制转换，推荐 std::bit_cast，涉及类型处理和基础知识。
- title: '## 11. 单元测试怎么测 private 函数'
  score: 7/10
  tags:
  - C++
  - 工程实践
  - 基础知识
  - 内容较主观
  explanation: 讨论单元测试中测试私有方法的实践和设计原则，属于工程实践和基础知识。内容较主观：包含个人观点。
- title: '## 12. `unique_ptr` 的大小'
  score: 8/10
  tags:
  - C++
  - 内存管理
