...
C++03 定义 POD 要求数据成员都是 public，到了 C++11 才允许 private / protected。而 Itanium ABI 是按照 C++03 对 POD 的定义来的，并且规定了 POD 的 tail padding 不能用于其他用途。<https://itanium-cxx-abi.github.io/cxx-abi/abi.html#pod> 的这句话：
...
补充说明：B1 和 B2 不受 Itanium ABI 约束。标准布局（POD 和标准布局等价）有一个要求“继承层级中仅有一个类具有非静态数据成员”（<https://zh.cppreference.com/w/cpp/language/classes>），B1 的继承层级有两个类有成员（即 A1 和 B1），B2 同理，不符合这个要求。
