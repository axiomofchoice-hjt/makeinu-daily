...
description: 本期日报包含 C++ 语言特性、内存管理和编译优化相关话题
__tags__:
- title: '## 1. 一个结构体因为对齐多出来的空地方，如果 memcpy 一下是不是未定义行为'
  score: 8/10
  tags:
  - C++
  - 内存管理
  - 类型
  - 基础知识
  explanation: 讨论可平凡复制类型的内存对齐和 memcpy 操作是否产生未定义行为，涉及内存管理和类型系统，属于基础知识。
- title: '## 2. 调用函数怎么编译成了 jmp 而不是 call'
  score: 7/10
  tags:
  - C++
  - 性能优化
  - 编译构建和调试
  - 进阶知识
  explanation: 解释尾调用优化的原理，说明编译器如何将 call 指令优化为 jmp 指令以提升性能，涉及编译优化技术，属于进阶知识。
- title: '## 3. 右值都存在寄存器，左值都存在内存，这个说法对吗'
  score: 8/10
  tags:
  - C++
  - 内存管理
  - 类型
  - 基础知识
  explanation: 澄清左值右值与寄存器内存存储的常见误解，说明编译器优化的实际行为，涉及内存管理和类型系统，属于基础知识。
- title: '## 4. 字符串字面量是左值'
  score: 6/10
  tags:
  - C++
  - 语法特性
  - 基础知识
  explanation: 说明字符串字面量的左值特性，可以对其取地址，涉及 C++ 语法特性，属于基础知识。
---
