...
p = 1 就是这题，需要单调栈 + 维护中位数（双堆或平衡树）。因为一堆点移动到一个点是中位数最优，保序回归就把数组分成多个块，每块求中位数即可。要想更快可以线性复杂度，就是这篇论文 <https://page.mi.fu-berlin.de/rote/Papers/pdf/Isotonic+regression+by+dynamic+programming.pdf>。
